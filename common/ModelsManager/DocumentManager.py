...
class DocumentManager:
    ...
    def SetPredictions(self, documents, predictions):
        for document, prediction in zip(documents, predictions):
            ...
            document.SetEntities(prediction, document.TokenizedText)
        return documents
        ...
        for tokenizedWord, predictedTag, predictedProb in zip(tokenizedText, predictedTags, predictedProbs):          
            if predictedTag[0:2] == 'B-':
                if entityData == []:
                    ...
                    entity = Models.Entity(text=entityManager.SetText(entityData), tag=entityManager.SetTag(entityData), probability=entityManager.SetProbability(entityData))
